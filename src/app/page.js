...
// Sabse zaroori line build error fix karne ke liye
...
async function getHomeData() {
  try {
    ...
    // Direct Database calls (Bina fetch ke - safe for build)
    ...
    <div className="bg-[#f8fafc] min-h-screen">
      ...
      <section className="bg-white pt-16 pb-24 border-b border-black/5">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex justify-center border border-black max-w-fit mx-auto overflow-hidden">
            <Link href="/products" className="bg-black text-white px-12 py-5 text-xs uppercase font-bold tracking-widest hover:bg-zinc-800 transition-colors border-r border-black">
              Shop All
            </Link>
            <Link href="/categories" className="bg-white text-black px-12 py-5 text-xs uppercase font-bold tracking-widest hover:bg-black hover:text-white transition-all">
              Catalog
            </Link>
          </div>
        </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
              {categories.map((category, index) => (
                <Link key={category._id} href={`/categories/${category.slug}`} className="group relative bg-[#f9f9f9] aspect-[4/3] flex flex-col justify-end p-8 border border-black/5 hover:border-black transition-all duration-500">
                  <span className="text-[10px] font-mono mb-2 text-gray-400 group-hover:text-black">/ 0{index + 1}</span>
                  <h3 className="text-2xl font-bold uppercase tracking-tight group-hover:pl-4 transition-all duration-500">{category.name}</h3>
